...
def n_grams(sentence):
    ...
    corpus_file = re.sub('\s+', ' ', corpus_file.read())
    my_markov = MarkovChain()
    my_markov.add_document(corpus_file)
    return my_markov.predictions(sentence.lower())
...
class MarkovChain:
    ...
    def predictions(self, string):
        if len(self.lookup_dict) > 0:
            tokens = string.split(" ")
            if len(tokens) == 1:
                predictions = self.oneword(string.split(" "))
                for prediction in predictions:
                    self.prediction_list.append(prediction[0])
            elif len(tokens) == 2:
                predictions = self.twowords(string.split(" "))
                for prediction in predictions:
                    self.prediction_list.append(prediction[0])
            elif len(tokens) == 3:
                predictions = self.threewords(string.split(" "))
                for prediction in predictions:
                    self.prediction_list.append(prediction[0])
            elif len(tokens) > 3:
                predictions = self.morewords(string.split(" "))
                for prediction in predictions:
                    self.prediction_list.append(prediction[0])
